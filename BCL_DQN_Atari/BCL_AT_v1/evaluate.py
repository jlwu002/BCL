...
from adv_attacks.RI_FGSM import RI_FGSM_Attack
...
parser.add_argument('--RI_FGSM',
                   dest='RI_FGSM',
                   action='store_true',
                   help='evaluate against projected RI-FGSM attack')
parser.add_argument('--RI_FGSM_Multi',
                   dest='RI_FGSM_Multi',
                   action='store_true',
                   help='evaluate against projected RI-FGSM (Multi) attack')
parser.add_argument('--RI_FGSM_Multi_T',
                   dest='RI_FGSM_Multi_T',
                   action='store_true',
                   help='evaluate against projected RI-FGSM (Multi-T) attack')
...
def attack_eval(curr_model, env, args, epsilon=1e-4, attack_type='FGSM', record=False):
    assert attack_type in ('FGSM', 'PGD', 'RI_FGSM', 'RI_FGSM_Multi', 'RI_FGSM_Multi_T'), 'Invalid attack type'
    ...
    elif attack_type == 'RI_FGSM': #FGSM with random initialization
        attack = RI_FGSM_Attack(m, attack_type)
        attack_config = {"epsilon": epsilon, 'steps': 1}
    elif attack_type == 'RI_FGSM_Multi': #RI-FGSM (Multi): FGSM with random initialization, sample N = 1000 random starts
        attack = RI_FGSM_Attack(m, attack_type)
        attack_config = {"epsilon": epsilon, 'steps': 1}
    elif attack_type == 'RI_FGSM_Multi_T': #RI-FGSM (Multi-T): FGSM with random initialization, sample N = 1000 random starts (select/target the lowest Q value that can be reached)
        attack = RI_FGSM_Attack(m, attack_type)
        attack_config = {"epsilon": epsilon, 'steps': 1}
...
if __name__ == '__main__':
    ...
    if args.fgsm:
        ...
        for epsilon in epsilons:
            ...
            curr_rewards = []
            for i in range(20):
                reward = attack_eval(model, env, args, epsilon/255, 'FGSM')
    ...
    if args.RI_FGSM:
        np.save('figures/{}/RI_FGSM_epsilons.npy'.format(save_name), epsilons)
        rewards = []
        for epsilon in epsilons:
            print(epsilon)
            curr_rewards = []
            for i in range(20):
                env = atari_env(args.env, env_conf, args)
                set_seed(i)
                reward = attack_eval(model, env, args, epsilon/255, 'RI_FGSM')
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        ...
        plt.ylabel('reward')
        #plt.xscale('log')
        np.save('figures/{}/RI_FGSM.npy'.format(save_name), rewards)
        plt.savefig('figures/{}/RI_FGSM.png'.format(save_name))
        plt.close()
        print(epsilons)
        print("RI_FGSM Average:", np.mean(rewards, axis=1))
        print("RI_FGSM SEM:", np.std(rewards, axis = 1)/np.sqrt(20))
            
    if args.RI_FGSM_Multi:
        np.save('figures/{}/RI_FGSM_Multi_epsilons.npy'.format(save_name), epsilons)
        rewards = []
        for epsilon in epsilons:
            print(epsilon)
            curr_rewards = []
            for i in range(20):
                env = atari_env(args.env, env_conf, args)
                set_seed(i)
                reward = attack_eval(model, env, args, epsilon/255, 'RI_FGSM_Multi')
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        ...
        plt.ylabel('reward')
        #plt.xscale('log')
        np.save('figures/{}/RI_FGSM_Multi.npy'.format(save_name), rewards)
        plt.savefig('figures/{}/RI_FGSM_Multi.png'.format(save_name))
        plt.close()
        print(epsilons)
        print("RI_FGSM_Multi (N=1000) Average:", np.mean(rewards, axis=1))
        print("RI_FGSM_Multi (N=1000) SEM:", np.std(rewards, axis = 1)/np.sqrt(20))
    
    if args.RI_FGSM_Multi_T:
        np.save('figures/{}/RI_FGSM_Multi_T_epsilons.npy'.format(save_name), epsilons)
        rewards = []
        for epsilon in epsilons:
            print(epsilon)
            curr_rewards = []
            for i in range(20):
                env = atari_env(args.env, env_conf, args)
                set_seed(i)
                reward = attack_eval(model, env, args, epsilon/255, 'RI_FGSM_Multi_T')
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        ...
        plt.ylabel('reward')
        #plt.xscale('log')
        np.save('figures/{}/RI_FGSM_Multi_T.npy'.format(save_name), rewards)
        plt.savefig('figures/{}/RI_FGSM_Multi_T.png'.format(save_name))
        plt.close()
        print(epsilons)
        print("RI_FGSM_Multi_T (N=1000) Average:", np.mean(rewards, axis=1))
        print("RI_FGSM_Multi_T (N=1000) SEM:", np.std(rewards, axis = 1)/np.sqrt(20))
    ...
    if args.pgd:
        ...
        for epsilon in epsilons:
            ...
            for i in range(20):
                env = atari_env(args.env, env_conf, args)
                set_seed(i)
                reward = attack_eval(model, env, args, epsilon/255, 'PGD')
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        ...
        plt.close()
        print(epsilons)
        print("PGD Average:", np.mean(rewards, axis=1))
        print("PGD SEM:", np.std(rewards, axis = 1)/np.sqrt(20))
        #print("Total PGD Average:", np.mean(np.mean(rewards, axis=1)))
    ...
    if args.nominal:
        ...
        plt.close()
        print("Nominal Average:", np.mean(rewards))
        print("Nominal SEM:", np.std(rewards)/np.sqrt(20))

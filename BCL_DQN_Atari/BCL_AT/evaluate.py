    ...
    with torch.no_grad():
        for _ in range(args.max_episode_length):
            ...
            if args.gpu_id >= 0:
                with torch.cuda.device(args.gpu_id):
                    state = state.cuda()
            output = curr_model.forward(state)
            #output = value + advs
...
if __name__ == '__main__':
    ...
    if args.RI_FGSM:
        ...
        for epsilon in epsilons:
            ...
            for i in range(args.num_episodes):
                ...
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        plot_results(epsilons, rewards, save_name, 'RI_FGSM')
        print(epsilons)
        print("RI-FGSM Average:", np.mean(rewards, axis=1))
        print("RI-FGSM SEM:", np.std(rewards, axis=1)/np.sqrt(args.num_episodes))
    ...
    if args.RI_FGSM_Multi:
        ...
        for epsilon in epsilons:
            ...
            for i in range(args.num_episodes):
                ...
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        plot_results(epsilons, rewards, save_name, 'RI_FGSM_Multi')
        print(epsilons)
        print("RI_FGSM_Multi (N=1000) Average:", np.mean(rewards, axis=1))
        print("RI_FGSM_Multi (N=1000) SEM:", np.std(rewards, axis=1)/np.sqrt(args.num_episodes))
    ...
    if args.RI_FGSM_Multi_T:
        ...
        for epsilon in epsilons:
            ...
            for i in range(args.num_episodes):
                ...
                curr_rewards.append(reward)
            rewards.append(curr_rewards)
        plot_results(epsilons, rewards, save_name, 'RI_FGSM_Multi_T')
        print(epsilons)
        print("RI_FGSM_Multi_T (N=1000) Average:", np.mean(rewards, axis=1))
        print("RI_FGSM_Multi_T (N=1000) SEM:", np.std(rewards, axis=1)/np.sqrt(args.num_episodes))
    ...
    if args.pgd:
        ...
        print("PGD SEM:", np.std(rewards, axis=1)/np.sqrt(args.num_episodes))
    ...
    if args.nominal:
        ...
        print("Nominal SEM:", np.std(rewards)/np.sqrt(args.num_episodes))

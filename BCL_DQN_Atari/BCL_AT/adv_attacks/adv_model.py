...
class PytorchModel(Model):
    ...
    def gradient_RI_FGSM(self, data, label, epsilon):
        """
        Calculate the RI-FGSM gradient of the cross-entropy loss w.r.t the image.
        Args:
            data(numpy.ndarray): input data with shape (size, height, width,
            channels).
            label(int): Label used to calculate the gradient.
        ...
        mm = nn.Softmax(dim = 1)
        delta = torch.zeros_like(scaled_data).uniform_(-epsilon, epsilon).to(self._device)
        delta.requires_grad = True
        
        #deal with multiple outputs
        try:
            output=self.predict_tensor(scaled_data + delta).to(self._device)
        except(AttributeError):
            output = self.predict_tensor(scaled_data + delta)[1].to(self._device)
    ...
    def gradient_RI_FGSM_Multi(self, data, label, epsilon):
        """
        Calculate the RI-FGSM (Multi) gradient of the cross-entropy loss w.r.t the image.
        Args:
            data(numpy.ndarray): input data with shape (size, height, width,
            channels).
            label(int): Label used to calculate the gradient.
        ...
        scaled_data = torch.from_numpy(scaled_data).to(self._device)
        scaled_data_batch = torch.cat([scaled_data]*1000)
        label = np.array([label])
        label = torch.from_numpy(label).to(self._device)
        label_batch = torch.cat([label]*1000)

        delta = torch.zeros_like(scaled_data_batch).uniform_(-epsilon, epsilon).to(self._device)
        delta.requires_grad = True
        
        #deal with multiple outputs
        try:
            output=self.predict_tensor(scaled_data_batch + delta).to(self._device)
        except(AttributeError):
            output = self.predict_tensor(scaled_data_batch + delta)[1].to(self._device)
        
        ce = nn.CrossEntropyLoss()
        loss=-ce(output, label_batch)
        ...
        try:
            output_orig = self.predict_tensor(scaled_data).to(self._device)
        except(AttributeError):
            output_orig = self.predict_tensor(scaled_data)[1].to(self._device)
            
        try:
            output_label =self.predict_tensor(scaled_data_batch - delta).to(self._device)
            output_label = torch.argmax(output_label, dim = 1)
            
        except(AttributeError):
            output_label = self.predict_tensor(scaled_data_batch - delta)[1].to(self._device)   
            output_label = torch.argmax(output_label, dim = 1)
            
        idx_sel = torch.where(output_label != label)[0]
        if len(idx_sel) == 0:
            idx = 0
        else:
            idx = idx_sel[0]

        grad = delta[idx:idx+1].cpu().numpy().copy()

        return grad.reshape(scaled_data.shape)
        
    def gradient_RI_FGSM_Multi_T(self, data, label, epsilon):
        """
        Calculate the RI-FGSM (Multi-T) gradient of the cross-entropy loss w.r.t the image.
        Args:
            data(numpy.ndarray): input data with shape (size, height, width,
            channels).
            label(int): Label used to calculate the gradient.
        ...
        action_order = torch.sort(output_orig)[1][0]
        ...
        for action in action_order:
            idx = (output_label == action).nonzero(as_tuple=True)[0]
            if len(idx) != 0:
                idx = idx[0]
                break
            else:

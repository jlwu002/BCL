...
#Jumper environment: use 10-step PGD to generate adversarial examples
def calculate_adv_img_Jumper(model, images, labels, eps, device):
    mm = nn.Softmax(dim = 1)
    rel_step_size = 0.2
    iters = 10
    images = torch.clone(images.type(torch.FloatTensor)).to(device)
...
#FruitBot environment: use RI-FGSM to generate adversarial examples
def calculate_adv_img_FruitBot(curr_model, data, epsilon, device):
    mm = nn.Softmax(dim = 1)
    
    state = torch.clone(data.type(torch.FloatTensor)).to(device)
    ...
    adv_images = data + gradient
    eta = torch.clamp(adv_images - data, min=-epsilon, max=epsilon)
    states_pert = torch.clamp(data + eta, min=0, max=255).detach_()
    return states_pert
    ...
    def _update(self, dataset):
        ...
        for batch in _yield_minibatches(
            ...
            if self.epsilon_scheduler:
                attack_eps = self.epsilon_scheduler.get_eps(0, self.n_updates) + self.eps_start/255.0
                
                attack_eps = attack_eps*255.0

                if self.env_name == 'jumper':
                    states_pert = calculate_adv_img_Jumper(self.model, states, actions, attack_eps, device)
                elif self.env_name == 'fruitbot':
                    states_pert = calculate_adv_img_FruitBot(self.model, states, attack_eps, device)
                else:
                    raise ValueError("Invalid environment name!")
                
                logits_pert, _ = self.model(states_pert)


                onehot_labels = torch.zeros(logits_pert.shape).to(device)
                onehot_labels[range(actions.shape[0]), actions] = 1

                upper_logits = logits_pert*onehot_labels + logits*(1-onehot_labels)
                upper_log_probs = torch.distributions.Categorical(logits=upper_logits).log_prob(actions)
                lower_logits = logits*onehot_labels + logits_pert*(1-onehot_labels)
